...
public class RomiChallenge4 extends SequentialCommandGroup{
    ...
    private CommandBase getCommandsToDriveAlongPath() {
        SequentialCommandGroup moppingCommands = new SequentialCommandGroup(
            TimedCommand.create(TIME_BEFORE_FIRST_MOP),
            TimedCommand.create(TIME_BETWEEN_FIRST_AND_SECOND_MOP),
            TimedCommand.create(TIME_BETWEEN_SECOND_AND_THIRD_MOP)
        );
