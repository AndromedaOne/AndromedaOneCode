...
public abstract class PathGeneration2d extends PathGeneratorBase {
    ...
    public PathGeneration2d(WaypointsBase waypoints, WaypointWithHeading initialWaypoint, WaypointWithHeading finalPoint){
    ...
    protected void generatePathForNextRelativeToStartWaypoint(Waypoint waypoint) {
        // this doesn't do anything because the path will be fully generated in getGeneratedPath.
    }
    ...
    protected CommandBase getGeneratedPath() {
        ...
        Trajectory trajectory = TrajectoryGenerator.generateTrajectory(initialPose2dPoint, interiorPoints, finalPose2dPoint, config);
        ResetOdometryCommand resetOdometryCommand = new ResetOdometryCommand(initialPose2dPoint);
        SequentialCommandGroup sequentialCommandGroup = new SequentialCommandGroup(resetOdometryCommand, getRamsete(trajectory));
    ...
    private Pose2d convertInchesWaypointToMeterPose2d(WaypointWithHeading waypoint){
        ...
        return new Pose2d(Units.inchesToMeters(waypoint.getY()), Units.inchesToMeters(waypoint.getX()), new Rotation2d(waypoint.getHeading()));
    }
