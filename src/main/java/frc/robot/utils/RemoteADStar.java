...
public class RemoteADStar implements Pathfinder {
  ...
  private final AtomicReference<List<PathPoint>> currentPath = new AtomicReference<>(
      new ArrayList<>());
  ...
  public RemoteADStar() {
    ...
    dynamicObsPub = nt.getDoubleArrayTopic("/PPLibCoprocessor/RemoteADStar/dynamicObstacles")
        .publish();

    pathPointsSub = nt.getDoubleArrayTopic("/PPLibCoprocessor/RemoteADStar/pathPoints")
        .subscribe(new double[0], PubSubOption.keepDuplicates(true), PubSubOption.sendAll(true));

    nt.addListener(pathPointsSub, EnumSet.of(NetworkTableEvent.Kind.kValueAll), event -> {
      double[] pathPointsArr = pathPointsSub.get();

      List<PathPoint> pathPoints = new ArrayList<>();
      for (int i = 0; i <= pathPointsArr.length - 2; i += 2) {
        pathPoints
            .add(new PathPoint(new Translation2d(pathPointsArr[i], pathPointsArr[i + 1]), null));
      }
