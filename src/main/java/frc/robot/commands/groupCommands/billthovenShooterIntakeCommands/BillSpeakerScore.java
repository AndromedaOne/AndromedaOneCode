...
public class BillSpeakerScore extends SequentialCommandGroup4905 {
  ...
  public BillSpeakerScore(BillArmRotateBase armRotate, BillEndEffectorPositionBase endEffector,
    ...
    if (distance == SpeakerScoreDistanceEnum.AWAY) {
      /*
       * addCommands( new TurnToTargetUsingGyro(m_driveTrain, () -> m_wantedID, () ->
       * 0, false, m_photonVision), new PauseRobot(40, m_driveTrain), new
       * ParallelDeadlineGroup4905( new BillDistanceSpeakerScore(armRotate,
       * endEffector, feeder, shooter, distance, useSmartDashboard), new
       * PauseRobot(Robot.getInstance().getSubsystemsContainer().getDriveTrain())));
       */
    } else {
