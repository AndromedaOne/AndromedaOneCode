...
public class SwerveSetpointGeneratorOLD {
  ...
  public SwerveSetpointGeneratorOLD(final SwerveDriveKinematics kinematics) {
    m_kinematics = kinematics;
    m_numModules = kinematics.toSwerveModuleStates(new ChassisSpeeds()).length;
  }
  ...
  private double findRoot(Function2d func, double x_0, double y_0, double f_0, double x_1,
    ...
    if (iterations_left < 0 || PathPlannerUtils.epsilonEquals(f_0, f_1)) {
      return 1.0;
    }
  ...
  protected double findSteeringMaxS(double x_0, double y_0, double f_0, double x_1, double y_1,
      double f_1, double max_deviation) {
  ...
  protected double findDriveMaxS(double x_0, double y_0, double f_0, double x_1, double y_1,
      double f_1, double max_vel_step) {
    double diff = f_1 - f_0;
   ...
   */
  public SwerveSetpointOLD generateSetpoint(final SwerveKinematicLimits limits,
      final SwerveSetpointOLD prevSetpoint, ChassisSpeeds desiredState, double dt) {
    ...
    if (PathPlannerUtils.epsilonEquals(desiredState, new ChassisSpeeds())) {
      need_to_steer = false;
    ...
    double[] prev_vx = new double[m_numModules];
    double[] prev_vy = new double[m_numModules];
    Rotation2d[] prev_heading = new Rotation2d[m_numModules];
    double[] desired_vx = new double[m_numModules];
    double[] desired_vy = new double[m_numModules];
    Rotation2d[] desired_heading = new Rotation2d[m_numModules];
    ...
    if (all_modules_should_flip
        && !PathPlannerUtils.epsilonEquals(prevSetpoint.chassisSpeeds, new ChassisSpeeds())
        && !PathPlannerUtils.epsilonEquals(desiredState, new ChassisSpeeds())) {
    ...
    // lazy).
    List<Optional<Rotation2d>> overrideSteering = new ArrayList<>(m_numModules);
    ...
    for (int i = 0; i < m_numModules; ++i) {
      ...
      if (PathPlannerUtils.epsilonEquals(prevSetpoint.moduleStates[i].speedMetersPerSecond, 0.0)) {
        ...
        if (PathPlannerUtils.epsilonEquals(desiredModuleState[i].speedMetersPerSecond, 0.0)) {
        ...
        if (numStepsNeeded <= 1.0) {
          ...
          overrideSteering.set(i, Optional.of(prevSetpoint.moduleStates[i].angle.rotateBy(Rotation2d
    ...
    for (int i = 0; i < m_numModules; ++i) {
      ...
      double s = min_s * findDriveMaxS(prev_vx[i], prev_vy[i], Math.hypot(prev_vx[i], prev_vy[i]),
          vx_min_s, vy_min_s, Math.hypot(vx_min_s, vy_min_s), max_vel_step);
      min_s = Math.min(min_s, s);
    }
    ...
    for (int i = 0; i < m_numModules; ++i) {
      ...
      final var deltaRotation = prevSetpoint.moduleStates[i].angle.unaryMinus()
    ...
    return new SwerveSetpointOLD(retSpeeds, retStates);
  }
}