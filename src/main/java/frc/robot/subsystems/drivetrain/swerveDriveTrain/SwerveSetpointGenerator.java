 ...
 *
 * <p>
 * Takes a prior setpoint, a desired setpoint, and outputs a new setpoint that
 * respects all the kinematic constraints on module rotation and wheel
 * velocity/torque, as well as preventing any forces acting on a module's wheel
 * from exceeding the force of friction.
 */
public class SwerveSetpointGenerator {
   ...
   * @param maxSteerVelocityRadsPerSec The maximum rotation velocity of a swerve
  ...
  /**
   * Generate a new setpoint with explicit battery voltage. Note: Do not
   * discretize ChassisSpeeds passed into or returned from this method. This
   * method will discretize the speeds for you.
   *
   * @param prevSetpoint              The previous setpoint motion. Normally,
   *                                  you'd pass in the previous iteration
   *                                  setpoint instead of the actual
   *                                  measured/estimated kinematic state.
   * @param desiredStateRobotRelative The desired state of motion, such as from
   *                                  the driver sticks or a path following
   *                                  algorithm.
   * @param constraints               The arbitrary constraints to respect along
   *                                  with the robot's max capabilities. If this
   *                                  is null, the generator will only limit
   *                                  setpoints by the robot's max capabilities.
   * @param dt                        The loop time.
   * @param inputVoltage              The input voltage of the drive motor
   *                                  controllers, in volts. This can also be a
   *                                  static nominal voltage if you do not want
   *                                  the setpoint generator to react to changes
   *                                  in input voltage. If the given voltage is
   *                                  NaN, it will be assumed to be 12v. The input
   *                                  voltage will be clamped to a minimum of the
   *                                  robot controller's brownout voltage.
   * @return A Setpoint object that satisfies all the kinematic/friction limits
   *         while converging to desiredState quickly.
   */
  public SwerveSetpoint generateSetpoint(final SwerveSetpoint prevSetpoint,
      ChassisSpeeds desiredStateRobotRelative, PathConstraints constraints, double dt,
      double inputVoltage) {
    ...
    if (constraints != null) {
      Translation2d vel = new Translation2d(desiredStateRobotRelative.vxMetersPerSecond,
      ...
      desiredStateRobotRelative = new ChassisSpeeds(vel.getX(), vel.getY(),
          MathUtil.clamp(desiredStateRobotRelative.omegaRadiansPerSecond,
              -constraints.maxAngularVelocityRadPerSec(),
              constraints.maxAngularVelocityRadPerSec()));
    }

    SwerveModuleState[] desiredModuleStates = config
        .toSwerveModuleStates(desiredStateRobotRelative);
    ...
    for (int m = 0; m < config.numModules; m++) {
      ...
      desired_vx[m] = desiredModuleStates[m].angle.getCos()
          * desiredModuleStates[m].speedMetersPerSecond;
      desired_vy[m] = desiredModuleStates[m].angle.getSin()
          * desiredModuleStates[m].speedMetersPerSecond;
      ...
      if (all_modules_should_flip) {
        double required_rotation_rad = Math
            .abs(prev_heading[m].unaryMinus().rotateBy(desired_heading[m]).getRadians());
    ...
    for (int m = 0; m < config.numModules; m++) {
      ...
      if (epsilonEquals(prevSetpoint.moduleStates()[m].speedMetersPerSecond, 0.0)) {
        ...
        var necessaryRotation = prevSetpoint.moduleStates()[m].angle.unaryMinus()
        ...
        if (numStepsNeeded <= 1.0) {
          ...
          overrideSteering.set(m,
              Optional.of(prevSetpoint.moduleStates()[m].angle.rotateBy(Rotation2d
                  .fromRadians(Math.signum(necessaryRotation.getRadians()) * max_theta_step))));
          min_s = 0.0;
        }
        continue;
      }
      ...
      // We do this by changing max_theta_step to the maximum change in heading over
      // dt
      // that would create a large enough radius to keep the centripetal force under
      // the
      // friction force.
      ...
      double s = findSteeringMaxS(prev_vx[m], prev_vy[m], prev_heading[m].getRadians(),
          desired_vx[m], desired_vy[m], desired_heading[m].getRadians(), max_theta_step);
    ...
    for (int m = 0; m < config.numModules; m++) {
      ...
      double currentDraw = config.moduleConfig.driveMotor.getCurrent(Math.abs(lastVelRadPerSec),
          inputVoltage);
      ...
      double moduleTorque;
      if (epsilonEquals(prevSpeed, 0.0) || (prevSpeed > 0 && desiredSpeed >= prevSpeed)
    ...
    if (constraints != null) {
      ...
      chassisAngularAccel = MathUtil.clamp(chassisAngularAccel,
    ...
    for (int m = 0; m < config.numModules; m++) {
      ...
      double vx_min_s = min_s == 1.0 ? desired_vx[m]
          : (desired_vx[m] - prev_vx[m]) * min_s + prev_vx[m];
      double vy_min_s = min_s == 1.0 ? desired_vy[m]
    ...
    double angularAccel = (retSpeeds.omegaRadiansPerSecond
        - prevSetpoint.robotRelativeSpeeds().omegaRadiansPerSecond) / dt;
    ...
    for (int m = 0; m < config.numModules; m++) {
      ...
      final var deltaRotation = prevSetpoint.moduleStates()[m].angle.unaryMinus()
          .rotateBy(retStates[m].angle);
      ...
      accelFF[m] = (retStates[m].speedMetersPerSecond
          - prevSetpoint.moduleStates()[m].speedMetersPerSecond) / dt;
   ...
   */
  public SwerveSetpoint generateSetpoint(final SwerveSetpoint prevSetpoint,
      ChassisSpeeds desiredStateRobotRelative, PathConstraints constraints, Time dt,
      Voltage inputVoltage) {
    return generateSetpoint(prevSetpoint, desiredStateRobotRelative, constraints, dt.in(Seconds),
        inputVoltage.in(Volts));
  }

  /**
   * Generate a new setpoint. Note: Do not discretize ChassisSpeeds passed into or
   * returned from this method. This method will discretize the speeds for you.
   *
   * <p>
   * Note: This method will automatically use the current robot controller input
   * voltage.
   *
   * @param prevSetpoint              The previous setpoint motion. Normally,
   *                                  you'd pass in the previous iteration
   ...
   *         while converging to desiredState quickly.
   */
  public SwerveSetpoint generateSetpoint(SwerveSetpoint prevSetpoint,
      ChassisSpeeds desiredStateRobotRelative, PathConstraints constraints, double dt) {
    return generateSetpoint(prevSetpoint, desiredStateRobotRelative, constraints, dt,
   ...
   */
  public SwerveSetpoint generateSetpoint(final SwerveSetpoint prevSetpoint,
      ChassisSpeeds desiredStateRobotRelative, Time dt, Voltage inputVoltage) {
    return generateSetpoint(prevSetpoint, desiredStateRobotRelative, null, dt.in(Seconds),
        inputVoltage.in(Volts));
  }
   ...
   */
  public SwerveSetpoint generateSetpoint(SwerveSetpoint prevSetpoint,
      ChassisSpeeds desiredStateRobotRelative, double dt) {
    return generateSetpoint(prevSetpoint, desiredStateRobotRelative, null, dt,
        RobotController.getInputVoltage());
  }
   ...
   */
  public SwerveSetpoint generateSetpoint(SwerveSetpoint prevSetpoint,
      ChassisSpeeds desiredStateRobotRelative, Time dt) {
    return generateSetpoint(prevSetpoint, desiredStateRobotRelative, null, dt.in(Seconds),
        RobotController.getBatteryVoltage());
  }
  ...
  private static double findSteeringMaxS(double x_0, double y_0, double theta_0, double x_1,
      double y_1, double theta_1, double max_deviation) {
  ...
  private static double findDriveMaxS(double x_0, double y_0, double x_1, double y_1,
      double max_vel_step) {
    // Derivation:
